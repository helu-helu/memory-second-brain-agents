...
### 1. (Không Cần Docker) Qdrant Local Mode
Hệ thống sử dụng Qdrant Local Engine chạy trực tiếp bằng file (tại thư mục `db/`), giúp tiết kiệm RAM và không yêu cầu cài đặt Docker.
...
### 3. Chạy Server và Dashboard (Phase 2)
Sau khi cài đặt xong, bạn có thể chạy song song 2 thành phần của hệ thống:
...
**Terminal 2 (Chạy Giao diện Quản trị Streamlit):**
```bash
streamlit run dashboard.py
```
*(Nếu là lần đầu tiên chạy, hệ thống sẽ tự động quét thư mục `docs/` để lập chỉ mục. Vui lòng chờ 1-2 phút).*
...
Dự án đã được trang bị bộ kiểm thử tự động sử dụng `pytest`. Các bài test được thiết kế dưới dạng **Pure Unit Test** (sử dụng Mock) nên chạy rất nhanh và không cần kết nối tới Qdrant/Gemini thực.

Để chạy kiểm thử, dùng lệnh:
```bash
