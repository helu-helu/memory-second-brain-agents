...
**Note**: On small-screen devices running Android 12 or newer versions, a resizable application supports multi-window mode based on the [minimum width and minimum height](class-PlayerSettingsAndroid.html#minWidthHeight) of the activity. A non-resizable application doesn’t support multi-window mode on small-screen devices.

## Application and Configuration APIs

The [AndroidApplication API](../ScriptReference/Android.AndroidApplication.html) provides information on your Android application while it runs on a device. You can use this API to perform the following actions:
...
To enable exemption for your game application, Unity provides the **App Category** Player setting. This setting replaces the [**androidIsGame**](../ScriptReference/PlayerSettings.Android-androidIsGame.html) API, which is no longer supported.
...
The **App Category** must be set to **Game**, if you’re using [Adaptive Performance](adaptive-performance/adaptive-performance.html) or [GameMode API](../ScriptReference/Android.AndroidGame.GameMode.html). To do so, follow these steps:
...
* For existing projects with [**androidIsGame**](../ScriptReference/PlayerSettings.Android-androidIsGame.html) set to false, the **App Category** remains disabled.
