...
See in [Glossary](Glossary.html#Scripts) within the scope of the analyzer and analyzes the code in those scripts according to the rules in the analyzer.

Any scripts that are within the same assembly definition as an analyzer are in that analyzer’s scope. For analyzers in the root level of the `Assets` folder, Unity considers all files in the project to be in scope. For more information about scope, refer to [Analyzer scope and rule set files](analyzer-scope-and-diagnostics.html).
...
* [Analyzer scope and rule set files](analyzer-scope-and-diagnostics.html)
