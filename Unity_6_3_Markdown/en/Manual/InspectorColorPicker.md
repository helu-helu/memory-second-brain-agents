...
See in [Glossary](Glossary.html#Inspector)** window displays color properties as swatches. The swatch:
...
* High Dynamic Range (HDR) color: For example, for the emission color in the [standard shader](shader-StandardShader.html). For more information, refer to the [HDR color picker reference](hdr-landing.html).
* Gradient: For example, for the color over lifetime in the [Particle System](PartSysColorOverLifeModule.html)A component that simulates fluid entities such as liquids, clouds and flames by generating and animating large numbers of small 2D images in the scene. [More info](class-ParticleSystem.html)  
...
2. Click on a **pixel**The smallest unit in a computer image. Pixel size depends on your screen resolution. Pixel lighting is calculated at every screen pixel. [More info](ShadowPerformance.html)  
   See in [Glossary](Glossary.html#pixel) to select its color. You can use any pixel, in or outside of the Unity Editor.
...
* The [Particle System](PartSysColorOverLifeModule.html) uses gradients to change **particle**A small, simple image or mesh that is emitted by a particle system. A particle system can display and move particles in great numbers to represent a fluid or amorphous entity. The effect of all the particles together creates the impression of the complete entity, such as smoke. [More info](class-ParticleSystem.html)  
  See in [Glossary](Glossary.html#particle) color over their lifetime.
* The [Line Renderer](class-LineRenderer.html)A component that takes an array of two or more points in 3D space and draws a straight line between each one. You can use a single Line Renderer component to draw anything from a simple straight line to a complex spiral. [More info](class-LineRenderer.html)  
  See in [Glossary](Glossary.html#LineRenderer) uses gradients to change the color of the line along its length.

You can’t change a single color property to a gradient property in the **Inspector** window, but you can use a script to override the property. For more information, refer to [Gradient class](../ScriptReference/Gradient.html).
...
* Classic blend: A [linear interpolation](linear-color-space-landing.html) between stops.
...
* [HDR color picker reference](hdr-landing.html)
* [linear interpolation](linear-color-space-landing.html)
