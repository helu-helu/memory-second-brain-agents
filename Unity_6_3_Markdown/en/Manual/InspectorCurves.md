...
A curve is a line graph that shows the response (on the y-axis) to the value of an input (on the x-axis) as that input changes. The Unity Editor uses curves in many components. For more information about using curves, refer to [Edit Animation curves](EditingCurves.html).
