...
During the project export process, Unity’s [incremental build pipeline](building-introduction.html#incremental-build-pipeline) might overwrite any changes you make in the exported project. If you want your changes to persist:
...
2. Create a new **GameObject**The fundamental object in Unity scenes, which can represent characters, props, scenery, cameras, waypoints, and more. A GameObject’s functionality is defined by the Components attached to it. [More info](class-GameObject.html)  
   See in [Glossary](Glossary.html#GameObject) and name it **SendMessageReceiver**.
