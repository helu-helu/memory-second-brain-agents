...
See in [Glossary](Glossary.html#compression) formats and access patterns.
...
* **Loading cache**: Stores recently accessed pages of an AssetBundle file. Use [`AssetBundle.memoryBudgetKB`](../ScriptReference/AssetBundle-memoryBudgetKB.html) to control its size.
...
To test the impact that TypeTrees have on the size of AssetBundles, build them with and without TypeTrees disabled and compare their sizes. Use [`BuildAssetBundleOptions.DisableWriteTypeTree`](../ScriptReference/BuildAssetBundleOptions.DisableWriteTypeTree.html) to disable TypeTrees in an AssetBundle.
...
The methods [`GetAllAssetNames`](../ScriptReference/AssetBundle.GetAllAssetNames.html) and [`GetAllScenePaths`](../ScriptReference/AssetBundle.GetAllScenePaths.html) expose the table of contents map.
...
Each asset has its own preload table. For example, a **prefab**An asset type that allows you to store a GameObject complete with components and properties. The prefab acts as a template from which you can create new object instances in the scene. [More info](Prefabs.html)  
See in [Glossary](Glossary.html#prefab)’s preload table includes entries for all components attached to the prefab, referenced materials and textures, and any other assets the prefab uses.
...
[CRC checks](AssetBundles-Integrity.html) for LZ4 files can affect load times because the entire file needs to be decompressed. This uses minimal memory because Unity decompresses each chunk individually instead of the entire file at once.

CRC checks for LZMA files incur no additional overhead, because full decompression is inherent. For more information, refer to [Downloading AssetBundles](AssetBundles-Integrity.html).
...
* **`PersistentManager.Remapper`**: Tracks relationships between instance IDs and source files. Can use noticeable memory for AssetBundles containing very large Prefabs (object hierarchies) as it never reduces in size once allocated. Objects in **scenes**A Scene contains the environments and menus of your game. Think of each unique Scene file as a unique level. In each Scene, you place your environments, obstacles, and decorations, essentially designing and building your game in pieces. [More info](CreatingScenes.html)  
  See in [Glossary](Glossary.html#Scene) are not tracked here.
...
* [`BuildAssetBundleOptions.DisableLoadAssetByFileName`](../ScriptReference/BuildAssetBundleOptions.DisableLoadAssetByFileName.html)
* [`BuildAssetBundleOptions.DisableLoadAssetByFileNameWithExtension`](../ScriptReference/BuildAssetBundleOptions.DisableLoadAssetByFileNameWithExtension.html)

Refer to the documentation for the `name` parameter in [`AssetBundle.LoadAssetAsync`](../ScriptReference/AssetBundle.LoadAssetAsync.html) for more information.
...
* [Analyzing AssetBundles](assetbundles-analyze.html)
