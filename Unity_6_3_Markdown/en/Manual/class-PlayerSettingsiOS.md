...
Icon files can be any image file [supported by Unity](texture-choose-format-by-platform.html), in the form of a [Texture2D](../ScriptReference/Texture2D.html) asset. If an icon Texture is missing, Unity scales the icon Texture with the nearest size, with a preference towards using larger Textures.
...
| **Property** | **Description** |
| --- | --- |
| **Enable Internal Profiler (Deprecated)** | This feature is deprecated and will be removed in a future version of Unity. Use the [Profiler window](ProfilerWindow.html) instead.  The Profiler collects application performance data and prints a report to the console. The report has the number of milliseconds each Unity subsystem takes to execute on each frame, averaged across 30 frames. |
...
| **Enable CrashReport API** | Enable a custom crash reporter to capture crashes. You can use scripts to access crash logs via the [CrashReport](../ScriptReference/CrashReport.html) API. |
...
| **Texture compression format** | Set ASTC as the default texture compression format for iOS. For more information, refer to [texture compression format overview](texture-choose-format-by-platform.html#ios-and-tvos). |
...
| **Variant map for app slicing** | Expand the Variant map for app slicing section to find the list of variant names configured in scripting. For more information on variants, refer to [App slicing](ios-app-slicing.html).  **Note**: Enable **Use on-demand resources** to view this property. |
